...
async function updateAllLeadersProgress() {
  ...
  try {
    ...
    console.log(
      `[ProgressSync] Found ${leaders.length} verified active leaders.`,
    );

    for (const leader of leaders) {
      console.log(
        `[ProgressSync] Updating progress for: ${leader.name} (${leader.uid})`,
      );
      ...
      for (const quest of quests) {
        ...
        const pIndex = leader.progress.findIndex(
        ...
        if (pIndex !== -1) {
          ...
          const newValues = await resolveMetricValue(
            quest.metric,
            leader.uid,
            nEntities,
          );
          ...
          if (quest.type === "discrete" && quest.entityLimit > 1) {
            ...
            if (completedEntities > (progress.value || 0)) {
              ...
              progress.overallProgress = Math.min(
                (completedEntities / nEntities) * 100,
                100,
              );
      ...
      for (const p of leader.progress) {
        if (!quests.some((q) => q._id.toString() === p.questId.toString())) {
          console.warn(
            `[ProgressSync] Quest ${p.questId} missing from API response for leader ${leader.name}`,
          );
        }
      }
      console.log(
        `[ProgressSync] Finished updating progress for: ${leader.name} (${leader.uid})`,
      );
    }
  } catch (error) {
