...
async function handleTags({ tags, memoryId, userId }) {
  try {
      ...
      validTags.map(async (tag) => {
        try {
          if (tag.type === "people") {
            ...
            const user = await User.findById(tag._id, { memoryRequests: 1 });
            if (user) {
              user.memoryRequests.addToSet(memoryObjectId);
              await user.save();
            }
          } else if (tag.type === "club") {
            const club = await Club.findById(tag._id, { memoryRequests: 1 });
            if (club) {
              club.memoryRequests.addToSet(memoryObjectId);
              await club.save();
            }
          }
        } catch (innerErr) {
    ...
    if (validPeopleTags.length > 0) {
      await User.findByIdAndUpdate(
        userId,
        { $addToSet: { memoryList: { $each: validPeopleTags } } },
        { new: true }
      );
    }
  } catch (error) {
...
async function memoryCreationSecondaryActions({
  ...
  try {
    ...
    const users = await User.find(
      { _id: { $in: validPeopleTags } },
      { pushToken: 1, unreadNotice: 1 }
    ).lean();
...
const createMemory = async (req, res) => {
  try {
    ...
    //  Get user info
    const creatorInfo = await User.findById(userId, { name: 1, image: 1 });
    ...
    if (visibility !== "private") {
      await handleTags({ tags, memoryId: memory._id, userId });
    }
...
const editMemory = async (req, res) => {
  try {
    ...
    if (
      ...
      await handleTags({ tags: updates.tags, memoryId, userId });
...
const saveMemoryRequest = async (req, res) => {
  try {
    ...
    const updatedUser = await User.findByIdAndUpdate(
      userId,
      { $pull: { memoryRequests: memory._id } },
      { new: true }
    );
...
const fetchMemoryCollections = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId).select("memoryList role");
    ...
    if (user.role === "Creator") {
      macbeaseContributions = await getLatestTwoImages({ senderId: userId });
    }
