...
const findValidQuests = async (req, res) => {
  try {
    const userId = mongoose.Types.ObjectId(req.user.id); // Ensure correct ObjectId format
...
const fetchQuests = async(req,res) => {
  try {
    ...
    const quests = await getValidQuestsForUser(mongoose.Types.ObjectId(userId));
...
module.exports = { createQuest, findValidQuests,fetchQuests };
