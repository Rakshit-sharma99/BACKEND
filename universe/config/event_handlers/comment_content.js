...
const comment_content = async (messageValue) => {
  try {
    ...
    for (const field of requiredFields) {
      if (!data[field]) {
        console.warn(`Missing field ${field} in comment_content payload`);
        return;
      }
    }
    ...
    console.error("❌ Failed to process comment content topic", error);
  }
};

module.exports = { comment_content };
