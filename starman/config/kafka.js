...
const connectProducer = async () => {
  console.log("⏳ Starman: Connecting Kafka producer...");
  while (true) {
    try {
      await producer.connect();
      connected = true;
      console.log("✅ Starman Kafka Producer connected");
      break;
    } catch (error) {
      console.error(
        "❌ Starman Kafka Producer connection failed. Retrying in 5s...",
        error.message
      );
