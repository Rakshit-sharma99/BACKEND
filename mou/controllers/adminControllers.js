...
const getPendingMOUs = async (req, res) => {
  ...
  try {
    ...
    const mous = await MOU.find({ status: { $in: ["draft", "ready"] } }).sort({ createdAt: -1 });
...
const setMOUParameters = async (req, res) => {
  ...
  console.log(`➡️  [ADMIN API] PATCH /admin/${mouId}/parameters called with body:`, req.body);
  try {
    const { commissionRate, platformFee, paymentTerms, cancellationPolicy, liabilityClause, customClauses, custom } = req.body;
    ...
    if (mou.status === "signed" || mou.status === "voided") {
      return res.status(StatusCodes.BAD_REQUEST).json({ success: false, error: `Cannot modify MOU in ${mou.status} state` });
    }
    ...
    console.log(`✅  [ADMIN API] Parameters set for MOU ${mouId}, status updated to 'ready'`);
...
const sendMOU = async (req, res) => {
  ...
  try {
    ...
    if (mou.status !== "ready") {
      console.warn(`⚠️  [ADMIN API] Cannot send MOU ${mouId}, current status is ${mou.status}`);
      return res.status(StatusCodes.BAD_REQUEST).json({ success: false, error: `MOU must be in 'ready' state to send (current: ${mou.status})` });
    }

    // Prepare tab values for DocuSign
    const tabValues = {
      event_name: mou.eventName,
      club_name: mou.clubName,
      commission_rate: mou.parameters.commissionRate,
      platform_fee: mou.parameters.platformFee,
      payment_terms: mou.parameters.paymentTerms,
      cancellation_policy: mou.parameters.cancellationPolicy,
      creator_name: mou.creatorName
    };

    console.log(`⏳  [ADMIN API] Calling DocuSign to create envelope for MOU ${mouId}...`);
    const envelopeId = await docusign.createEnvelope({
      signerEmail: mou.creatorEmail || "dummy@example.com", // Ensure email exists or fetch from User service
      signerName: mou.creatorName || "Event Creator",
      clientUserId: mou.creatorId.toString(),
      tabValues
    });
    console.log(`✅  [ADMIN API] DocuSign envelope created successfully: ${envelopeId}`);
    ...
    res.status(StatusCodes.OK).json({ success: true, status: "sent", envelopeId });
  } catch (error) {
...
const createMOUDraft = async (req, res) => {
  console.log("➡️  [INTERNAL API] POST /internal/draft called with body:", req.body);
  try {
    const { eventId, clubId, creatorId, eventName, clubName, creatorName, creatorEmail, universityId } = req.body;

    const existingMou = await MOU.findOne({ eventId });
    if (existingMou) {
      console.warn(`⚠️  [INTERNAL API] MOU already exists for event ${eventId}`);
      return res.status(StatusCodes.OK).json({ success: true, mou: existingMou });
    }
    ...
    console.log(`✅  [INTERNAL API] MOU Draft created successfully for event ${eventId} -> MOU ID: ${newMou._id}`);
